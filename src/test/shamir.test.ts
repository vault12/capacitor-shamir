...
test('restoreShard', async () => {
  const { shardsBase64 } = await prepareInMemoryStuff();
  const idx = 2;
...
async function prepareFileStuff() {
  const tempDir = 'temp';
...
test('restoreFromFileShardsToFile', async () => {
  const { shardPaths, srcBase64, absRestoredPath } = await prepareFileStuff();
...
test('restoreFileShard', async () => {
  const { shardPaths, absDstShardPath } = await prepareFileStuff();
  const idx = 2;
  const restoredPath = await new Promise<string>((resolve, reject) => {
